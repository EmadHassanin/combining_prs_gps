ibd_all <- self_reported %>% filter(code_selfreported == 1461 |
                                    code_selfreported == 1462 |
                                    code_selfreported == 1463) %>% 
   mutate(age_selfreported = if_else(age_selfreported < 0 , mean(age_selfreported), age_selfreported )) %>% 
   full_join(icd9 %>% filter(str_detect(code_icd9 , "^(555[0-9])")),
             by=c("eid", "new")) %>% 
   full_join(icd10 %>% filter(str_detect(code_icd10 , "^(K51[0-9])")),
             by=c("eid", "new"))  %>% 
   ...
   select(eid,code_icd10,code_icd9,code_selfreported,age,type) %>% 
   mutate(pheno = 1) 
